...
function CalcBattleNoInRank()
	...
	if nTaskValue >= 250000 then
		SetTask(704, 6 * song_or_liao)	-- 元帅
	elseif nTaskValue >= 120000 then
		SetTask(704, 5 * song_or_liao)	-- 将军
	elseif nTaskValue >= 80000 then
		SetTask(704, 4 * song_or_liao)	-- 先锋
	elseif nTaskValue >= 50000 then
...
function CalcBattleRank()
	local flag,nCurGs = GetRealmType();
	if nCurGs == 1 then
		return 0;
	end
	...
	local t =
	{
		{999, 250000, 6, 0},	--1 开放军功到门槛直接升级军衔（均设定1000）
		{50, 120000, 5, 0},	--8个将军
		{100, 80000, 4, 0},	--10个先锋
		{500,50000,3,0},
		{1000,15000,2,0},
