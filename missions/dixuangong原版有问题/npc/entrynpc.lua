...
function dxg_entry_npc_main()
	...
	tinsert(selTab,"◆武器特效/#show_equip_shop(3109)");
	tinsert(selTab,"◆天使神兵/#show_equip_shop(3108)");
	tinsert(selTab,"◆天使神装/#show_equip_shop(3103)");
...
---流派数≥3
	...
	for i = 1, 1 do
		...
		nRetCode, szRetName	= gf_team_check_level(nTeamID, 50, 99);
		if 1 ~= nRetCode then
			bResult	= 0;
			if szRetName then
				gf_team_message(nTeamID, szRetName.."进入地玄宫失败（队伍中有队员不足50级）");
				szMsg = szMsg..szRetName.."进入地玄宫失败（队伍中有队员不足50级）";
			end
			break;
		end
