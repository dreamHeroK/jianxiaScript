...
function dxg_entry_npc_main()
	...
	tinsert(selTab,"◆武器特效/#show_equip_shop(5009)");
	tinsert(selTab,"◆天使神兵/#show_equip_shop(5008)");
	tinsert(selTab,"◆天使神装/#show_equip_shop(5003)");
	--tinsert(selTab,"◆地玄宫介绍/dxg_info");
	tinsert(selTab,"◆勇闯地玄宫/#join_dxg(0)");
	--tinsert(selTab,"◆ 地玄宫每日任务/dxg_daily_task_menu");
	--tinsert(selTab,"◆领取天骄令/cbwmadw");
	tinsert(selTab,"◆结束对话/nothing");
	Say(szTitle, getn(selTab), selTab)
end
function cbwmadw()
	if GetItemCount(2,97,236) <=10 then
...
---流派数≥3
	...
	for i = 1, 1 do
		...
		nRetCode, szRetName	= gf_team_check_level(nTeamID, 10, 99);
		if 1 ~= nRetCode then
			bResult	= 0;
			if szRetName then
				gf_team_message(nTeamID, szRetName.."进入地玄宫失败（队伍中有队员不足10级）");
				szMsg = szMsg..szRetName.."进入地玄宫失败（队伍中有队员不足10级）";
			end
			break;
		end
		...
		if 1 > nRetCode then   --- 个人修改为0，实际数字为3！ 
			bResult = 0;
		...
		if 1 > nRetCode then  --- 个人修改为0，实际数字为4！
			bResult = 0;
			szMsg = szMsg.."队伍人数小于4人";
			break;
		end
	end
