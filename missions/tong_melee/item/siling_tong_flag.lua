...
function OnUse(nItem)
	...
	local npcIndex = CreateNpc("帮派蓝色阵旗", sTongName, nMapId, nX, nY);
	if npcIndex > 0 then
		SetNpcScript(npcIndex, "\\script\\missions\\tong_melee\\item\\siling_tong_flag.lua");
		SetNpcLifeTime(npcIndex, 2*60*60);
		local msg = format("%s在%s使用了%s, 帮会成员可以领取奖励！",GetName(), tMapName[nMapId], "帮会圣灵战旗");
		Msg2Global(msg);
...
function main()
	...
	local msg = "<color=green>帮会圣灵战旗<color>："..format("帮会<color=yellow>%s<color>在帮派会战中取得杰出的成就，所有帮会成员可以领取奖励。", sTongName);
	if GetTongName() ~= sTongName then
		tinsert(tSay, "我好像不是这个帮会成员/nothing");
		Say(msg, getn(tSay), tSay);
		return 0;
	end
	if gf_GetTaskByte(TM_TASKID_LINGSHI, 3) ~= 0 then
...
function get_award_now()
	...
	gf_SetTaskByte(TM_TASKID_LINGSHI, 3, 1, TASK_ACCESS_CODE_TONG_MELEE);
	local nLevel = GetLevel();
	gf_ModifyExp(nLevel*nLevel*nLevel*20);
	gf_EventGiveCustomAward(3, 150, 1);
	gf_EventGiveCustomAward(4, 200, 1);
	gf_AddItemEx2({2,95,5435, 10, 4}, "灵石宝箱", "帮会会战", "帮会圣灵战旗", 0, 1);
end
