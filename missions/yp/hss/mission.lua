...
--	yp_CreateMonster(tNpcs[m].jy2,tPos[m].jy2);
	local tHwPos = {
		...
		[5076] = {
			[1] = {{1836,3617},{1849,3603},{1823,3635},{1833,3622}},
			[2] = {{1789,3564},{1811,3582},{1827,3605},{1840,3621}},
			[3] = {{1738,3649},{1745,3638},{1753,3618},{1774,3610},{1783,3617}},
		},
		[5075] = {
	...
	yp_CreateMonster(tNpcs[m].cnhw2,tPos[m].cnhw2,tHwPos[m][2],AT_SM_PATROL);
	yp_CreateMonster(tNpcs[m].hrhw,tPos[m].hrhw,tHwPos[m][3],AT_SM_PATROL);
...
function stageFight.createFireBox()
	...
	for i=1,getn(tCreatTime) do
		if nTime >= tCreatTime[i] and nTime < tCreatTime[i]+15 and GetMissionV(this.mv12) == i then --15分钟内
			local m = this.msPosition:getMapID();
			local nNpcIdx = 0;
			for k = 1,getn(tPos[m].chd) do
				for j = 1,5 do
					nNpcIdx = CreateNpc("武林盟圣物","武林盟圣物",m, tPos[m].chd[k][1], tPos[m].chd[k][2],-1,1,1,150);
...
--			local strMsg = {
--				"圣灵城住持带领执事正在在拜火圣灵城传道授业，众位有心受教的侠士可前往拜访！",
--				"化生寺住持带领执事正在女真黄龙府传道授业，众位有心受教的侠士可前往拜访！",
--				"西夏征南将军带领征南军士正在西夏兴庆巡逻，请少侠速速回去支援！",
--			}
--			Msg2Global(strMsg[nType]);
--			--记下log
--			WriteLog(format("[西域传道] [Sucess] [Type:%d,Time:%d]",nType,nTime));
--		end
--	end
end
...
function pl_useSeed()
	...
	if nCamp == 0 then 
		Talk(1,"","只有加入势力才能使用此物品！");
		return 0;
	end
	if tMainMapID[nCamp][1] ~= nMapID then
		Talk(1,"","此物品只能在本势力地图使用！");
		return 0;
	end
	...
	if nDistance > 40 then
		Talk(1,"","此物品只能在太虚古树附近使用！");
		return 0;
	end
	if DelItem(2,95,914,1) == 1 then	
		local nIdx = CreateNpc("太虚树初始阶段",GetName().."的太虚古树",nMapID,nPosX,nPosY);
		...
		if nIdx ~= 0 then
			...
			tinsert(tPlantInfo,{nIdx,GetName(),GetTime(),1}); --索引 玩家名 种植时间 阶段
			tPlantInfo.n = nil;
		end
	end
end
...
function stageFight.checkPlant()
	...
	local nCurTime = GetTime();
	for k,v in tPlantInfo do --时间过了清掉
	...
	for i,v in tPlantInfo do
		...
		if nDisTime > g_nGrownTime and tPlantInfo[i][4] == 1 then
			...
			nNewIdx = CreateNpc("太虚树成长阶段",tPlantInfo[i][2].."的太虚古树",nMapID,nPosX,nPosY);
			if nNewIdx ~= 0 then
		...
		elseif nDisTime == g_nFinishTime - 30 or nDisTime == g_nFinishTime - 29 then
			if nCamp ~= 0 then
				Msg2Global(tMainMapID[nCamp][2].."势力的"..tPlantInfo[i][2].."的太虚古树即将成熟，快去领取奖励！");
			end
		elseif nDisTime > g_nFinishTime and tPlantInfo[i][4] == 2 then
			SetNpcLifeTime(tPlantInfo[i][1],0);
			nNewIdx = CreateNpc("太虚树最终阶段",tPlantInfo[i][2].."的太虚古树",nMapID,nPosX,nPosY);
			if nNewIdx ~= 0 then
				SetNpcScript(nNewIdx,"\\script\\missions\\yp\\tls\\plant_npc.lua");
...
tPackage = {
	[5074] = {"圣灵城包裹","圣灵城包裹","圣灵城住持留下的包裹","圣灵城住持",2,3,1},
	[5076] = {"化生寺包袱","化生寺包袱","化生寺住持留下的包袱","化生寺住持",1,3,2},
	[5075] = {"西夏军需箱子","西夏军需箱子","西夏征南将军留下的箱子","西夏征南将军",1,2,3},	
};	
...
--	gf_PrintTable(data,2)
	...
	if sNpcName == tNpcs[m].army1[2] then
		...
		this:Msg2MSAll(format("%s被其他势力击杀，所有当前地图的非本方势力玩家都将获得奖励！",tPackage[nMapID][4]));		 
...
--	print("stageFight:onNpcRemove",event, data)	
	local sNpcName = GetNpcName(data); --被击破npc的名字
	local m = this.msPosition:getMapID();
	if sNpcName == tNpcs[m].army1[2] then		
		...
		this:Msg2MSAll(format("%s没有被消灭，所有当前地图的本方势力玩家都将获得奖励！",tPackage[nMapID][4]));		 
	end
end
...
tbTBHSS.msOption.bForbitTeam = 1; --可以组队
tbTBHSS.msOption.nInitFightState = nil; --战斗状态
tbTBHSS.msOption.nRestoreFightState = nil; --战斗状态
tbTBHSS.msOption.bRestoreTempEffect = 0; --是否清除临时状态
