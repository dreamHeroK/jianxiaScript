...
----------------------------------------------------------------------
tbHLS = gf_CopyInherit(missionBase, tbHLS);

tbHLS.msOption.bForbitTeam = 1; --可以组队
tbHLS.msOption.nInitFightState = nil;  --战斗状态
tbHLS.msOption.nRestoreFightState = nil; --战斗状态
tbHLS.msOption.bRestoreTempEffect = 0; --是否清除临时状态
tbHLS.msOption.szInitDeahScript = "\\script\\missions\\yp\\player_death.lua";
tbHLS.msCamp.campType = tAllCamp;

tbHLS.msPhases[1] = firePhases;

tbHLS.msPosition.entryPos = {
	{1890,3186},
	{1884,3292},
--	{1593,3364},
};
tbHLS.msPosition.exitPos = {200,1381,2845};

function tbHLS:onInit()
----	print("tbSLY:onInit")
	firePhases:onInit();
	missionBase.onInit(self);
end

function tbHLS:onLeave()
--	print("tbSLY:onLeave")
...
tbHLS.msOption.OnPlayerJoin = function(self, nCamp)
	nCamp = nCamp or GetTask(TASK_FIELD_PVP_CAMP);
