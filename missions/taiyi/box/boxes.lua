...
T_BOX = {
	...
	--award
	[1] = {"盛世宝箱", "关卡铜宝箱", 1,"YXTY_tong1",},
	[2] = {"盛世宝箱", "关卡铜宝箱", 1,"YXTY_tong2",},
	[3] = {"盛世宝箱", "关卡铜宝箱", 1,"YXTY_tong3",},
	--award, consume
	[11] = {"盛世宝箱", "关卡银宝箱", 2,"YXTY_yin", "csm_2_tianjiaoling",},
	--free_award, ib_award,ib_consume,t_ib_consmue_ui,mapId
	[21] = {"盛世宝箱", "关卡金宝箱", 3,"YXTY_jin1_free", "YXTY_jin1_IB", "csm_2_tianjiaoling",{2,97,236,2},6075,},
	[22] = {"盛世宝箱", "关卡金宝箱", 3,"YXTY_jin2_free", "YXTY_jin2_IB", "csm_2_tianjiaoling",{2,97,236,2},6075,},
}
...
function _on_open_box_tong(tBox, npcIndex)
	...
	if nRetIndex > 0 then
		...
		_remove_boxes(npcIndex or 0) --删除箱子
...
function _on_open_box_yin(tBox, npcIndex)
	if GetNpcTempData(npcIndex, NPC_TEMP_DAYE_IDX) == 0 then
		...
		local szTitle = format("打开[%s] 需要 [%s]",tBox[2], szConsumeDesc)
		Say(szTitle,
			2, 
			format("\n%s/#_box_yin_imdaye(%d)", "选择", npcIndex),
			format("\n%s/nothing", "取消")
			)
		return
	else
	...
	if daye == PlayerIndex then
		...
		for i = 1,GetTeamSize() do
			...
			if PlayerIndex ~= OldPlayerIndex then
				...
				player_name = gf_FilterName(player_name,"|","-")
				player_name = gf_FilterName(player_name,":","-")
				tinsert(tSel, 1, format("%s%s/#_box_yin_loot_other(%d,%d)", "开启银宝箱 ",player_name, PlayerIndex, npcIndex))
			end
		end
		PlayerIndex = OldPlayerIndex
		tinsert(tSel, 1, format("%s/#_box_yin_loot_award(%d,%d)", "开启银宝箱", PlayerIndex, npcIndex))
	end
...
function _box_yin_loot_other(player_index, npcIndex)
	...
	if index_chg_chk_flag ~= 1 then
		Talk(1,"","<color=green>注意<color>: 请检查您所分配的人是否与您相同<color=yollow>团队<color> !")
		return
	end
	--分配对象的空间负重检测
...
function _box_yin_loot_award(player_index, npcIndex)
	...
	if nLootAwardId > 0 then
		...
		SetNpcTempData(npcIndex, NPC_TEMP_DAYE_IDX, 0)
		SetNpcTempData(npcIndex, NPC_TEMP_AWARD_ID, 0)
		_remove_boxes(npcIndex or 0)--删箱子
	else
