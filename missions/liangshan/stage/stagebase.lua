...
--		end
		...
		if nStage < 4 then
		--	if ret == 1 then
				gf_AddItemEx({2,95,808,2,1},"梁山银宝箱");
		--	end
		elseif nStage > 4 then
			if ret == 1 then
				gf_AddItemEx({2,95,808,4,1},"梁山银宝箱");
			end
		end
		if nStage == 4 then
			if ret == 1 then
			...
			SendScript2VM("\\script\\misc\\huoyuedu\\huoyuedu.lua", format("AddHuoYueDu(%d)", 13));
		end
		if nStage == 8 then
			SendScript2VM("\\script\\misc\\huoyuedu\\huoyuedu.lua", format("AddHuoYueDu(%d)", 14));
		end
	end
