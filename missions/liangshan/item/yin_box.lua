...
function OnUse(nItem)
	Say("开启[梁山白银宝箱]需要消耗2个[天骄令]，你确定开启吗？",
		2,
		"\n 确定开启/#IamDaye("..nItem..")",
		"\n 不开启/nothing");
	return
	...
	if DelItem(OPEN_BOX_NEED[2],OPEN_BOX_NEED[3],OPEN_BOX_NEED[4],OPEN_BOX_NEED[5]) == 1 then
		...
		if DelItem(g,d,p, 1) == 1 then
			...
			if nNeedTianjiao >= T_NEED_TIANJIAOLING then
				...
				SaveNow(); -- 立即存盘
				local szNotice = format("恭喜%s在%s获得[%s]×%d", GetName(), "梁山群雄会", TIANGANGLING[4], nNum)
		...
		Talk(1, "", "你确定带了[天骄令]了吗？要开启<color=green>梁山白银宝箱<color>需要<color=yellow>2个<color>[天骄令]。");
	end
end
