...
--	szNpc_zhanglao = "藏剑长老",
--	szNpc_xiaojie = "藏剑小姐",
	szNpc_jiangbiehe = "剑僮叶量",
	szNpc_yeziqiu = "叶子丘",
	szNpc_renwoxing = "红印教主",
	szNpc_renyingying = "秋若水",

	szNpc_tongling = "无影刃萧九九",
...
-------一些公共数据---------------------------------------------
tNpcs = {
	wrx = {"吴仁杏","吴仁杏","\\settings\\static_script\\missions\\cangjianshanzhuang\\wrx_npc.lua",nil, CampPlayer,0},
	jtyl = {"剑僮叶量","剑僮叶量",thisFile,nil,CampPlayer,0},
	qrs = {"素仙秋若水", "秋若水", thisFile, nil, CampPlayer, 0},
	lf = {"红印教主剑冢", "红印教主", nil, nil, CampPlayer, 0},
	yzq = {"叶子丘剑冢", "叶子丘", thisFile, nil, CampPlayer, 0},
	cjzl = {"藏剑阁总管90", "藏剑阁总管", nil, nil, CampPlayer, 0},
	cjxj = {"大小姐90", "大小姐", nil, nil, CampPlayer, 0},
	nyzg = {"山庄内院总管90", "山庄内院总管", nil, nil, CampPlayer, 0},
	wyzg = {"藏剑山庄外院总管90", "藏剑山庄外院总管", nil, nil, CampPlayer, 0},

	cyfy = {"招魂柱", "鹰之印", nil, nil, CampEnemy, 0},
	qlfy = {"招魂柱", "龙之印", nil, nil, CampEnemy, 0},
	hffy = {"招魂柱", "凤之印", nil, nil, CampEnemy, 0},
	bhfy = {"招魂柱", "虎之印", nil, nil, CampEnemy, 0},
	yl = {"飞信鹰", "破阵鹰", nil, nil, CampPlayer, 0},
	ll = {"跟随小神龙", "破阵龙", nil, nil, CampPlayer, 0},
	fl = {"跟随小火凤", "破阵凤", nil, nil, CampPlayer, 0},
	hl = {"跟随小白虎", "破阵虎", nil, nil, CampPlayer, 0},

	boss1 = {"无影刃萧九九","无影刃萧九九", thisFile, nil, CampEnemy, 0},
	boss2 = {"机关盘龙柱","盘龙柱", thisFile, nil, CampEnemy, 0},
	boss3 = {"天剑叶风","天剑叶风", thisFile, nil, CampEnemy, 0},
	boss4 = {"地剑叶崩","地剑叶崩", thisFile, nil, CampEnemy, 0},
	boss5 = {"人剑叶梦","人剑叶梦", thisFile, nil, CampEnemy, 0},
	boss6 = {"魔剑传人叶量","魔剑传人叶量", nil, nil, CampEnemy, 0},--红印教主
	boss7 = {"幻影剑魂","幻影剑魂", thisFile, nil, CampEnemy, 0},--
};
...
function ttd_passStage(nStage,strBossName)
	...
	local get_fix_award = function(nCurStage)
    	...
    	gf_EventGiveAllAward(tbNormalAward[nCurStage].tbAward,"藏剑剑冢",format("WanJianZhong stage award %d",nCurStage),tAddRate);
--		AddItem(2,95,45027,5);	
--		Msg2Player("你获得了辉星碎片×5");		
--		AddLocalNews("恭喜["..GetName().."]在通关万剑冢时获得了辉星碎片×5");
	end
...
function wjz_DailyWeeklyTask(nType)
	if nType == 1 then
		if tGtTask:check_cur_task(85) == 1 and GetTask(VET_WJZ_DALIY_TASK_CROSS) ~= 1 then
			...
			Msg2Player(format("Nhi謒 v?ng祔 - th玭g quan 藏剑剑冢 %d/1 l莕 (ho祅 th祅h) ",GetTask(VET_WJZ_DALIY_TASK_CROSS)));
		...
		if tGtTask:check_cur_task(86) == 1 and GetTask(VET_WJZ_WEEK_TASK_CROSS) < 7 then
			...
			if GetTask(VET_WJZ_WEEK_TASK_CROSS) == 7 then
				Msg2Player(format("Nhi謒 v?tu莕 - th玭g quan 藏剑剑冢 %d/7 l莕 (ho祅 th祅h) ",GetTask(VET_WJZ_WEEK_TASK_CROSS)));
				AddRuntimeStat(4,16,0,1)
			else
				Msg2Player(format("Nhi謒 v?tu莕 - th玭g quan 藏剑剑冢 %d/7 l莕 ",GetTask(VET_WJZ_WEEK_TASK_CROSS)));
			end
		end
	elseif nType == 2 then
