...
g_NpcName = "<color=green>藏剑剑僮<color>:"
...
-- return 
	...
	if 1 == IsWanJianZhongMissionSystemOpen() then
		local strtab = {
    		"开启普通关卡/entry_jingli",
--			"天骄开启关卡/#entry_tianjiaoling(0)",
--    		"每日任务/jt_talk_3",
--    		"每周任务/jt_talk_4",
 --   		"师门4套兑换/yes_open6",
    		"结束对话/do_nothing",
    		}
    	local szMsg = g_NpcName.."特来此恭迎天下英雄，请问有何事需要本弟子效劳。"
    	Say(szMsg,getn(strtab),strtab)
	else
		local strtab = {
    		"结束对话/do_nothing",
    		}
    	Say(g_NpcName..format("入口 %s未打开请打开它在来闯关", "万剑冢"),
		getn(strtab),strtab)
	end
...
function entry_tianjiaoling(bSure)
	if not bSure or 0 == bSure then
		local tSay = {
			...
			Say(format(" 需要消耗 <color=red>%d 个 %s (可以在御街买到)<color> 同意吗?",N_JOIN_TIANJIAOLING_NEED, T_TIANJIAOLING[4]),getn(tSay),tSay)
		return
	end
	jt_talk_1(1)
end
...
--do return 1 end
	...
	if getn(tState.lv) ~= 0 then
		nResult = 0;
		msg = msg .. "<color=red>成员未达到等级<color>";
	...
	if nRouteCount < 1 then   --流派
		nResult = 0;
		msg = msg .. "\n<color=red>成员流派不够 2 种<color>\n";   
	end
	if getn(tState.noRoute) ~= 0 then
		nResult = 0;
		msg = msg .. "\n<color=red>有未加入派系的成员<color>";
	...
	if tState.count < 1 then    --队伍人数
		nResult = 0;
		msg = msg .. "<color=red>只能超过 2 个成员才能进入<color>\n";
	end
...
function jt_talk_2()
	Talk(1,"main",g_NpcName.."加流派的玩家等级70以上才有进入万剑冢的权限。队伍成员2人以下的阵营不能进入关卡 ")
end
