...
--				"新手指南/ask_reason3",				
	...
	tSay = {
			...
			-- "常用物品/#Auto_Menu(0,0)",				
			-- "随机5级韵灵/rnad_rl",	
			"☆终级材料999☆/bagualingshiking",
			"☆八卦☆盘/Get_banguanpan",
			--"拆解令/get_cjl",
			--"洗髓丹/get_xsd",
			"☆转职☆/zhuanzhiye",
			--"》》☆耀阳☆套装+10/Get_YaoYang_10",
		       -- "☆青龍☆神装+15/bagualingshizb",		
		       -- "☆白虎☆神装+15/baifuzhangbei",					
			--"》》☆光照☆烈武八卦/Get_LW_10",
			--"》》☆将军☆装备/Get_item_zc",
			--"》》神行宝典/get_sx",
			"☆帮会相关/cbwmadw",
		...
		if 3 == nRoute then
			tinsert(tSay, "\n获得破魔咒/Give_ZhuzhuZhouzhou")
		end
		if 6 == nRoute then
			tinsert(tSay, "\n机关暗器/Give_JiguanAnqi")
		end
		if 8 == nRoute then
			tinsert(tSay, "\n舍利金丹/Give_Dandan")
		end		
		...
		if 20 == nRoute then
			tinsert(tSay, "\n获得封尸符/Give_Fengshifu")
		end
		if 21 == nRoute then
			tinsert(tSay, "\n获得蛊/Give_GuGu")
		end
		if 30 == nRoute then
			tinsert(tSay, "\n翠烟灵女小貂/ling_nv_xiaodiao")
		end
		if Get_Admin()== 1 then
			tinsert(tSay, "》》GM控制台/gmgaojipin1")
		end
		tinsert(tSay, "☆师门☆决要/Get_Book_JueYao")
		--tinsert(tSay, "\n☆帮会☆物品/get_bh")
	-- tinsert(tSay, "下一页/Index_ext");	
	tinsert(tSay, "☆清空☆背包/ClearBag");	
	tinsert(tSay, "退出/nothing");
	Say(g_main, getn(tSay), tSay);
	end
end
...
function Auto_Menu_fg(nWz,nPage)
	--菜单名称，物品名称 物品代码。数量
	 local tMenu={
		{"一闪妖煌_铁骨",2,1,1250,100},
		{"一闪妖煌_百战",2,1,1251,100},
		{"一闪妖煌_灼日",2,1,1252,100},
		{"一闪妖煌_俊逸",2,1,1253,100},
		{"一闪妖煌_桃李",2,1,1254,100},
		{"一闪妖煌_名宿",2,1,1255,100},
		{"一闪妖煌_落霞",2,1,1256,100},
		{"一闪妖煌_凌霄",2,1,1257,100}, 
		{"一闪妖煌_焚云",2,1,1258,100}, 
		{"三耀龙爔_星奕",2,1,1259,100}, 
...
local tSay = {}
	tSay = {
			...
			--"天地装备/#Set_hzb(1)",
			"☆披风☆令牌/#Set_hzb(2)",
			"☆武器☆发光/#Auto_Menu_fg(0,0)",
			 -- "☆耀阳☆套装+10/Get_YaoYang_10",
		      "☆青龙☆神装+15/bagualingshizb",		
		       "☆白虎☆神装+15/baifuzhangbei",	
		        "☆朱雀☆神装+15/baifuzhangbei1",	
			"☆隐龙☆武器+15/Set_xywq",
		     -- "☆天使4☆神装+15/baifuzhangbei2",		
			--"☆天使1☆武器+15/Set_xywq",			
			 -- "☆光照☆烈武八卦/Get_LW_10",
			 -- "☆将军☆装备+15/Get_item_zc",
		}	
	tinsert(tSay, "\n上一页/OnUse");
	tinsert(tSay, "\n退出/nothing");
	Say(g_main, getn(tSay), tSay);
...
function Set_hzb(tID)
	if tID == 1 then
		AddItem(0,152,36,1,3,-1,-1,-1,-1,-1,-1,-1,15);	
		AddItem(0,153,36,1,3,-1,-1,-1,-1,-1,-1,-1,15);
		AddItem(0,154,36,1,3,-1,-1,-1,-1,-1,-1,-1,15);	
	end
	if tID ==2 then
		AddItem(0,152,36,1,3,-1,-1,-1,-1,-1,-1,-1,15);	
		AddItem(0,153,36,1,3,-1,-1,-1,-1,-1,-1,-1,15);
...
function Set_xywq()
	local t_ITEM={
		...
		local nRoute = GetPlayerRoute();
		if nRoute == 8 then
			AddItem(t_ITEM[nRoute][2],t_ITEM[nRoute][3],t_ITEM[nRoute][4],1,1,7,16011,1,16160,1,16161,-1,15);		
			return
		end		
...
function Set_Banguanpan(nRoute)
	local tGOOD_ITEM = 
	{
		[1] = {"太虚八卦盘-乾",2,1,469,100},
		[2] = {"太虚八卦盘-坤",2,1,470,100},
		[3] = {"太虚八卦盘-坎",2,1,471,100},
		[4] = {"太虚八卦盘-兑",2,1,472,100},
		[5] = {"太虚八卦盘-离",2,1,473,100},
		[6] = {"太虚八卦盘-震",2,1,474,100},
		[7] = {"太虚八卦盘-艮",2,1,475,100},
		[8] = {"太虚八卦盘-巽",2,1,476,100},
		[9] = {"神木八卦",2,95,550,20},
		[10] = {"紫金八卦",2,95,551,20},
	}
...
--给八卦灵石
function bagualingshiking()
	AddItem(2,95,60000,999)
	AddItem(2,95,60001,999)
	AddItem(2,95,60002,999)
	AddItem(2,95,60005,999)
	Msg2Player("领取成功，4种极品材料各999个。")
...
function bagualingshizb()
	local	tTiandi = {
	{{"青龙盔",0,103,60001},{"青龙盔",0,103,60002},{"青龙盔",0,103,60003},{"青龙盔",0,103,60004}},
	{{"青龙甲 ",0,100,60001},{"青龙甲 ",0,100,60002},{"青龙甲 ",0,100,60003},{"青龙甲 ",0,100,60004}},
	{{"青龙装",0,101,60001},{"青龙装",0,101,60002},{"青龙装",0,101,60003},{"青龙装",0,101,60004}},
	{{"青龙玉",0,102,60001},{"青龙",0,102,60002},{"青龙",0,102,60003},{"青龙",0,102,60004}},
	{{"青龙玉",0,102,60021},{"青龙",0,102,60022},{"青龙",0,102,60023},{"青龙",0,102,60024}},
	{
	 [2]={"天地玄黄刀",0,3,5045}, --刀
	 [3]={"天地玄黄杖",0,8,5046},--禅
	 [4]={"天地玄黄手",0,0,5047},--WS	 
	 [6]={"天地玄黄器",0,1,5048},--唐门	
	 [8]={"天地玄黄剑",0,2,5049},--峨嵋佛家
	 [9]={"天地玄黄琴",0,10,5050},--峨嵋琴	
	 [11]={"天地玄黄手",0,0,5047},--丐帮掌
	 [12]={"天地玄黄棍",0,5,5051},	--丐帮棍
	 [14]={"天地玄黄剑",0,2,5249}, --武当道家
	 [15]={"天地玄黄笔",0,9,5052},--武当笔	 
	 [17]={"天地玄黄枪",0,6,5053},--杨门枪骑
	 [18]={"天地玄黄弓",0,4,5054},--杨门弓骑	 
	 [20]={"天地玄黄刃",0,7,5055},--五毒邪侠
	 [21]={"天地玄黄爪",0,11,5056},--五毒蛊师
	 [23]={"天地玄黄剑",0,2,5049},--昆仑天师
	 [29]={"天地玄黄扇",0,13,5057},--翠烟舞仙
	 [30]={"天地玄黄笛",0,12,5058}--翠烟灵女
	}};
	local nRoute = GetPlayerRoute();
	local nBody = GetBody();	
		...
		AddItem(tTiandi[4][nBody][2],tTiandi[4][nBody][3],tTiandi[4][nBody][4],1,1,-1,-1,-1,-1,-1,-1,-1);
		AddItem(tTiandi[5][nBody][2],tTiandi[5][nBody][3],tTiandi[5][nBody][4],1,1,-1,-1,-1,-1,-1,-1,-1);		
		--AddItem(tTiandi[6][nRoute][2],tTiandi[6][nRoute][3],tTiandi[6][nRoute][4],1,1,-1,-1,-1,-1,-1,-1,-1,15);
...
function baifuzhangbei()
	local	tTiandi = {
	{{"白虎盔",0,103,60005},{"白虎盔",0,103,60006},{"白虎盔",0,103,60007},{"白虎盔",0,103,60008}},
	{{"白虎甲 ",0,100,60005},{"白虎甲 ",0,100,60006},{"白虎甲 ",0,100,60007},{"白虎甲 ",0,100,60008}},
	{{"白虎装",0,101,60005},{"白虎装",0,101,60006},{"白虎装",0,101,60007},{"白虎装",0,101,60008}},
	{{"白虎玉",0,102,60005},{"白虎",0,102,60006},{"白虎",0,102,60007},{"白虎",0,102,60008}},
	{{"白虎玉",0,102,60025},{"白虎",0,102,60026},{"白虎",0,102,60027},{"白虎",0,102,60028}},
...
function baifuzhangbei1()
	local	tTiandi = {
	{{"朱雀盔",0,103,60009},{"朱雀盔",0,103,60010},{"朱雀盔",0,103,60011},{"朱雀盔",0,103,60012}},
	{{"朱雀甲 ",0,100,60009},{"朱雀甲 ",0,100,60010},{"朱雀甲 ",0,100,60011},{"朱雀甲 ",0,100,60012}},
	{{"朱雀装",0,101,60009},{"朱雀装",0,101,60010},{"朱雀装",0,101,60011},{"朱雀装",0,101,60012}},
	{{"朱雀玉",0,102,60009},{"朱雀",0,102,60010},{"朱雀",0,102,60011},{"朱雀",0,102,60012}},
	{{"朱雀玉",0,102,60029},{"朱雀",0,102,60030},{"朱雀",0,102,60031},{"朱雀",0,102,60032}},
...
function baifuzhangbei2()
	local	tTiandi = {
	{{"玄武盔",0,103,60013},{"玄武盔",0,103,60014},{"玄武盔",0,103,60015},{"玄武盔",0,103,60016}},
	{{"玄武甲 ",0,100,60013},{"玄武甲 ",0,100,60014},{"玄武甲 ",0,100,60015},{"玄武甲 ",0,100,60016}},
	{{"玄武装",0,101,60013},{"玄武装",0,101,60014},{"玄武装",0,101,60015},{"玄武装",0,101,60016}},
	{
	 [2]={"天地玄黄刀",0,3,8801}, --刀
	 [3]={"天地玄黄杖",0,8,8803},--禅
	 [4]={"天地玄黄手",0,0,8804},--WS	 
...
function ClearBagAllItem(bTag)
	...
	ClearItemInPos();	
	if GetItemCount(2,95,60016) < 1 and GetFreeItemRoom() > 0 then--这个是牌子
		AddItem(2,95,60016,1)			
	end
	if GetItemCount(2,95,60017) < 1 and GetFreeItemRoom() > 0 then--这个是牌子
...
function enter_mp(nRoute)
	...
	--	PlayerReborn(1, random(4));			--6转
		...
		SetStrengthMaxAddOn(3000) --力量上限+2000
		SetDexterityMaxAddOn(3000) --身法上限+2000
		SetVitalityMaxAddOn(3000) --外功上限+2000
		SetEnergyMaxAddOn(3000) --内功上限+2000
		SetObserveMaxAddOn(3000) --洞察上限+2000
		--PlaySound("\\sound\\sound_i016.wav");
		SetLevel(10, 1);								--设置等级
...
function cbwmadw()
  local szSay = {
    g_szTitle .. "帮会操作",
    "建 立 帮 会/TongOperation_Create",
    "\n帮 会 升 级/TongOperation_update",
    "\n关 闭/nothing",
  };
  SelectSay(szSay);
end
