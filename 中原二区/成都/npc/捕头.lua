...
Include("\\script\\task\\killer\\killyanzheng.lua");--验证机制
...
--                    "好，既然已危机百姓安危我等练武之人岂能坐视不理！/Task_yanzheng2",
					"任务流程/Task_Show",
					"任务背景/Task_Backdrop",
...
function bianjingmain()--此处新增接取任务新增验证机制	不验证则返回无法接取人物
main()
end



function Task_yanzheng()--此处新增接取任务新增验证机制	不验证则返回无法接取人物
abluemoon_2nd_go()
end
...
function Task_yanzheng2()
if GetTask(TASK_KILLER_DONUM_yanzheng) ~= 1     then
Task_yanzheng();
return 
else
Task_Confirm();
...
function Task_Confirm()
 --   Task_yanzheng2();
	...
	local nCoolTime = 1 - (nNowTime-nLastTime);
...
function Task_Finish()
	...
	if task_state == 2 then
		...
		SetTask(TASK_KILLER_DONUM_yanzheng,0);--完成任务，验证判断置0
