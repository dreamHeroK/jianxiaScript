...
function main()
	...
	local selTab = {};
	local szTitle = ""
	local szTitle = format("%s年轻人，看你精力旺盛，前方战线正需要大量矿石铸造兵器，不如加入我们，一起为国效力吧。使士当前气力值为：%d/%d。", strTitle, 600, 600)

	if (GetLifeSkillLevel(nGen, nSkillId) > 0) then
		tinsert(selTab,"进阶大师级挖矿技能/update_max_skill_level");
		tinsert(selTab,"我要了解采集的相关信息/Info");
		tinsert(selTab,"送我去挖矿地图/GotoWorld_Confirm");
		tinsert(selTab,"遗忘挖矿技能/forget_life_skill");
	else
		tinsert(selTab,"学习挖矿技能/luaLearnLifeSkill");
		tinsert(selTab,"我要了解采集的相关信息/Info");
	end
...
function luaLearnLifeSkill()
	...
	Say(strTitle.."<color=yellow>挖矿<color>将会成为你新的一个生活技能，你确定学习吗？",
    	2,
    	"是的，我要学/LearnYes",
    	"让我再考虑考虑/LearnNo")
end;
...
function LearnYes()
	if (GetLifeSkillLevel(nGen, nSkillId) > 0) then
		...
		if (LearnLifeSkill(nGen, nSkillId, 1, 79, 0)) then
			Talk(1,"", strTitle.."已经学会了[挖矿]技能。你可以雇佣采集工人，开始采集之旅。！")
        		Msg2Player("你学会了生活技能：挖矿")
    		end
	end
end
...
function update_max_skill_level()
	...
	if GetLifeSkillMaxLevel(nGen, nSkillId) > 79 then
		Talk(1,"", strTitle.."您已进阶大师级挖矿技能水平，不用再找我了！")
		return
	end

	local selTab = {};
	local szTitle = format("%s进阶大师级挖矿技能要求挖矿技能达到79级并消耗%d金，你确定要进阶吗？", strTitle, 1000)
...
function update_max_skill_level_ensure()
	if GetLifeSkillLevel(nGen, nSkillId) < 79 then
		Talk(1,"",format("%s目前挖矿技能小于%d级，先去历练一番，再来领会。！", strTitle, 79))
		return 0;
	end
	...
	Msg2Player("恭喜进阶大师级挖矿技能水平达到了99级");
...
function GotoWorld_Confirm()
	Say(strTitle.."请选择要去可以挂机的挖矿地点：",
		4,
		"前往石门洞(低级)/GotoWorld_Confirm1",
		"前往灵岩洞(中级)/GotoWorld_Confirm2",
		"前往悬星洞(高级)/GotoWorld_Confirm3",
		"稍后再说/exit_dialog")
end;
...
function GotoWorld_Confirm1()
	Say(strTitle.."您是否要去<color=yellow>石门洞<color>挖矿？那个地方有<color=yellow>精铁<color>, <color=yellow>百炼钢<color>, <color=yellow>乌金石<color>, <color=yellow>紫金<color>和<color=yellow>朱砂<color>，记得带上<color=yellow>神农丹<color>！",
		2,
		"我要去/#GotoWorld(711,1728,3383)",
		"稍后再说/exit_dialog")
end;

function GotoWorld_Confirm2()
	Say(strTitle.."您是否要去<color=yellow>灵岩洞<color>挖矿？那个地方有<color=yellow>寒铁<color>, <color=yellow>太白精金<color>, <color=yellow>玄铁<color>, <color=yellow>天青石<color>, <color=yellow>丹心砂<color>和<color=yellow>碧血砂<color>，记得带上<color=yellow>神农丹<color>！",
		2,
		"我要去/#GotoWorld(716,1694,3032)",
		"稍后再说/exit_dialog")
end;

function GotoWorld_Confirm3()
	Say(strTitle.."您是否要去<color=yellow>悬星洞<color>挖矿？那个地方有<color=yellow>玄石<color>, <color=yellow>黑钨石<color>, <color=yellow>黑钨精石<color>, <color=yellow>赤铁<color>, <color=yellow>云母<color>和<color=yellow>天灵砂<color>，记得带上<color=yellow>神农丹<color>！",
		2,
		"我要去/#GotoWorld(721,1688,3333)",
		"稍后再说/exit_dialog")
end;
