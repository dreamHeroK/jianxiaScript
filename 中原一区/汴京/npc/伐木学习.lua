...
--	 end
	...
	local selTab = {};
	local szTitle = ""
	local szTitle = format("%s前方战事不断，本材供应告急，正是咱们发财的大好时机。怎样：心动没？侠士当前气力值为：%d/%d。", strTitle, 600, 600)

	if (GetLifeSkillLevel(nGen, nSkillId) > 0) then
		tinsert(selTab,"进阶大师级伐木技能/update_max_skill_level");
		tinsert(selTab,"我要了解采集的相关信息/Info");
		tinsert(selTab,"送我去伐木地图/GotoWorld_Confirm");
		tinsert(selTab,"遗忘伐木技能/forget_life_skill");
	else
		tinsert(selTab,"学习伐木技能/luaLearnLifeSkill");
		tinsert(selTab,"我要了解采集的相关信息/Info");
	end
...
function luaLearnLifeSkill()
	...
	Say(strTitle.."<color=yellow>伐木<color>将会成为你新的一个生活技能，你确定学习吗？",
    	2,
    	"是的，我要学/LearnYes",
    	"让我再考虑考虑/LearnNo")
end
...
function LearnYes()
	if (GetLifeSkillLevel(nGen, nSkillId) > 0) then
		...
		if (LearnLifeSkill(nGen, nSkillId, 1, 79, 0)) then
			Talk(1,"", strTitle.."已经学会了[伐木]技能。你可以雇佣采集工人，开始采集之旅。！")
        		Msg2Player("你学会了生活技能：伐木")
    		end
	end
end
...
function update_max_skill_level()
	...
	if GetLifeSkillMaxLevel(nGen, nSkillId) > 79 then
		Talk(1,"", strTitle.."您已进阶大师级伐木技能水平，不用再找我了！")
		return
	end
	local selTab = {};
	local szTitle = format("%s进阶大师级伐木技能要求伐木技能达到79级并消耗%d金，你确定要进阶吗？", strTitle, 1000)
...
function update_max_skill_level_ensure()
	if GetLifeSkillLevel(nGen, nSkillId) < 79 then
		Talk(1,"",format("%s目前伐木技能小于%d级，先去历练一番，再来领会。！", strTitle, 79))
		return 0;
	end
	...
	Msg2Player("恭喜进阶大师级伐木技能水平达到了99级");
...
function GotoWorld_Confirm1()
	Say(strTitle.."您是否要去<color=yellow>兴安岭<color>砍伐树木？那里有很多<color=yellow>松木<color>, <color=yellow>杨木<color>, <color=yellow>杉木<color>和<color=yellow>铁木<color>，记得带上<color=yellow>神农丹<color>！",
		2,
...
function GotoWorld_Confirm2()
	Say(strTitle.."您是否要去 <color=yellow>会仙岭<color>砍伐树木？那里有很多<color=yellow>梧桐木<color>, <color=yellow>重木<color>, <color=yellow>铁樟木<color>和<color=yellow>铁桦木<color>，记得带上<color=yellow>神农丹<color>!",
		2,
...
function GotoWorld_Confirm3()
	Say(strTitle.."您是否要去 <color=yellow>苍龙岭<color>砍伐树木？那里有很多<color=yellow>铁线木<color>, <color=yellow>波罗木<color>, <color=yellow>檀木<color>, <color=yellow>菩提木<color>和<color=yellow>建木<color>，记得带上<color=yellow>神农丹<color>!  ",
		2,
