...
Include("\\script\\新藏剑山庄\\s5藏剑阁\\runtime_data_stat.lua")
...
--	SetTempRevPos(m,1698*32,3451*32); --临时重生点
...
function stageFight1:onNpcDeath(nNpcIDX)
    local nCurNum = GetMissionV(this.mv3)-1;--取件守卫数量
	local strNpcName = GetNpcName(nNpcIDX);
	if strNpcName == this.szNpc_tongling then--如果是boss
		ttd_passStage(1);
		local szMsg = format("通关成功")
    	this.msCamp:turnPlayer(0, HeadMsg2Player, szMsg);
	elseif strNpcName == this.szNpc_wuxingshouwei1 or strNpcName == this.szNpc_wuxingshouwei2 or strNpcName == this.szNpc_wuxingshouwei3  or strNpcName == this.szNpc_wuxingshouwei4 or strNpcName == this.szNpc_wuxingshouwei5 then
		SetMissionV(this.mv3,GetMissionV(this.mv3)-1);	--取件数量-1
...
function stageFight1.callBoss()--召唤boss
	self = stageFight1;
	local nchajianok = GetMissionV(this.mv9);----是否杀完五行守卫
	local nkaiqichajianok = GetMissionV(this.mv13);----是否杀完五行守卫
	local nCurNum = GetMissionV(this.mv3);--取件守卫数量
	if nCurNum <= 0 then         --如果杀完了取件守卫 开启里面得剑台
	if nkaiqichajianok < 1 then --如果没建立取件
	ttd_createSomeNpc(tNpcs.chajian1,tPos.chajian1);
	ttd_createSomeNpc(tNpcs.chajian2,tPos.chajian2);
	ttd_createSomeNpc(tNpcs.chajian3,tPos.chajian3);
	ttd_createSomeNpc(tNpcs.chajian4,tPos.chajian4);
	ttd_createSomeNpc(tNpcs.chajian5,tPos.chajian5);	
    this:Msg2MSAll("城里的大刀已经到了归来的嘈杂声中!");	
	SetMissionV(this.mv13,1);    --设置建立npc
	end
	end
	if GetMissionV(this.mv5) == 0 then--是否召唤过boss
		         if  nchajianok  <= 0 then --是否杀完五行守卫		
...
function stageFight1.callFish()--召唤杂鱼机关炉子
	...
	if nCurFish < 1 then --如果没建立取件
	ttd_createSomeNpc(tNpcs.qujian1,tPos.qujian1);
	ttd_createSomeNpc(tNpcs.qujian2,tPos.qujian2);
	ttd_createSomeNpc(tNpcs.qujian3,tPos.qujian3);
	ttd_createSomeNpc(tNpcs.qujian4,tPos.qujian4);
	ttd_createSomeNpc(tNpcs.qujian5,tPos.qujian5);	
	SetMissionV(this.mv3,5);    --此处改为取件守卫数量
	SetMissionV(this.mv2,1);    --设置建立npc
	SetMissionV(this.mv9,5);     --此处改为插入数量
	end
...
function stageFight1_2.gotoTwo(nType)
	...
	SendScript2VM("\\script\\新藏剑山庄\\s6紫竹林\\jt_npc.lua", "entry_jingli()");
