...
function hand_up()
	...
	if GetTaskTemp(TASK_FLAG_FOLLOW) == 1 then
		...
		Talk(1,"","太棒了，少侠获得了今日的汴京府南争夺镖旗奖励，明日再接再厉！")
		if GetGlbValue(ID_FLAG_NUMBER_BJ) == 0 then
			AddGlobalNews("汴京的5个东方龙珠首饰已被领走，汴京府南争夺镖旗活动已结束。")
			Msg2SubWorld("汴京的5个东方龙珠首饰已被领走，汴京府南争夺镖旗活动已结束。")
			return 0
		end
		Msg2SubWorld("汴京府南还可上缴"..flag_left.."个镖旗。")
	end
end
