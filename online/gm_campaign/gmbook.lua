...
GMsName = lonName
	...
	tinsert(tSay, "\n退出/gmgaojipin1");
	Say("GM卡：请选择封号时间?", getn(tSay), tSay);
...
function getofeng3(DonName) 
	...
	if neir == 1 then
	Msg2Player("GM卡：玩家 "..DonName.." 已经被封号并踢下线。封号信息:"..FID.."(注:0:永久手动解封，1:1天自动解封，7:7天后自动解封，14:14天后自动解封)");
	break
	end
	player = NextPlayer(player)	
	end
...
function SWPZDWJ()--刷物品给指定玩家

AskClientForString("getwupin","玩家名称",1,32,"输入玩家名");

end
function getwupin(lonName) 
	GMsName = lonName
	local player = {}
	local player = FirstPlayer()
	while (player > 0) do
	wz_Playername(player,lonName)
	player = NextPlayer(player)	
	end
end
function wz_Playername(nIndex,lonName)
	if nIndex == nil then
		nIndex = PlayerIndex;
	end;
	local nOldPlayer = PlayerIndex
...
--			"\n关闭/gmgaojipin",	
	...
	tinsert(tSay, "\n退出/gmgaojipin1");
	Say("GM卡：小战场是否执行。", getn(tSay), tSay);
...
--			"\n关闭/gmgaojipin",	
	...
	tinsert(tSay, "\n退出/gmgaojipin1");
	Say("GM卡：刷随机世界BOSS是否执行。", getn(tSay), tSay);
end
function suijiboss1 ()
      Random1=random(0,7)
	  Random2=random(0,4)
...
function ask_beilmz()  --北绿林盟主
	...
	if MapID > 999999 then
		Talk(1,"end_dialog","此地方无法召唤!")
		return
	end
	npcIndex = CreateNpc("北绿林盟主", "北绿林盟主",MapID,Att_X,Att_Y,-1,1,1,30)
	msg="据说称霸北方绿林的北绿林盟主突然被召唤出来，地图编号["..MapID.."]想来必有阴谋！"
		for i=1,3 do
		AddLocalNews(msg)
		end
...
function ask_qsyt()  --枪神赢天
	...
	npcIndex = CreateNpc("枪神赢天", "枪神赢天",MapID,Att_X,Att_Y,-1,1,1,30)
	msg="传说秦始皇的贴身侍卫枪神赢天突然被召唤出来，地图编号["..MapID.."]江湖上又将引起一阵血雨腥风！"
		for i=1,3 do
		AddLocalNews(msg)
		end
	SetNpcLifeTime(npcIndex, 7200)
...
function ask_lengxl()  --刷冷香菱
	...
	npcIndex = CreateNpc("冷香菱", "冷香菱",MapID,Att_X,Att_Y,-1,1,1,30)
	msg="相传数十年前江津村测头牌冷香凝魂魄突然被召唤出来，地图编号["..MapID.."]大家赶快去一睹芳容！"
		for i=1,3 do
		AddLocalNews(msg)
		end
	SetNpcLifeTime(npcIndex, 7200)
...
function ClearBagAllItem(bTag)
	...
	if GetItemCount(2,1,30644) < 1 and GetFreeItemRoom() > 0 then
		...
		AddItem(2,1,31261, 1,4);--清空背包后添加光武精破
