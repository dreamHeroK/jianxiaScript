...
function xqx_dialog_main()
	...
	tinsert(tbOpt, {"召唤箱子测试用 -->", tzyy_ty_box})
	...
	tinsert(tbOpt, {"(GM)重载脚本 -->", function()
		...
		local tbOpt ={}
		tinsert(tbOpt, {"只重载GM手册脚本", function()
...
function tzyy_ty_box()
	local nNpcIndex
	nNpcIndex = CreateNpc("盛世宝箱","关卡高级宝箱",GetWorldPos());
	SetNpcScript(nNpcIndex, "\\script\\missions\\taiyi\\box\\tong_box_1.lua")
end
...
function xqx_State_main()
	...
	tinsert(tbOpt, {"(BT)20分钟内免疫负面",function()
		CastState("state_paralysis_immune",100,1200*18)    --麻痹免疫
		CastState("state_vertigo_immune",100,1200*18)    --眩晕免疫
		CastState("state_fetter_immune",100,1200*18)    --定身免疫
		CastState("state_confusion_immune",100,1200*18)    --混乱免疫
		CastState("state_sleep_immune",100,1200*18)    --昏睡免疫
		CastState("state_attack_burst_dec_immune",100,1200*18)    --迟缓免疫
		CastState("state_slow_immune",100,1200*18)    --减速免疫
		CastState("state_knockdown_immune",100,1200*18)    --击倒免疫
		CastState("state_knockback_immune",100,1200*18)    --击退免疫
		CastState("state_knockdown_immune",100,1200*18)    --击倒免疫
		CastState("state_knockdown_immune",100,1200*18)    --击倒免疫
	end})
...
function xqx_Equipment_main()
	...
	tinsert(tbOpt, {"(DJ)获得套装（经典套） -->", function()
		...
		tinsert(tbOpt, {"九州系列首饰和武器..", AskClientForNumber, {"xqx_Equipment_getJiuZhou", 0, maxRefined, "输入强化度(0-"..maxRefined..")"}})
		tinsert(tbOpt, {"返回主菜单", xqx_dialog_main})
		tinsert(tbOpt, {"结束对话"})
		xqx.CreateNewSayEx(szTitle, tbOpt)
	end})
	tinsert(tbOpt, {"获得套装（新套装） -->", function()
		...
		tinsert(tbOpt, {"(BT)洪荒套..", AskClientForNumber, {"xqx_Equipment_getHongHuang", 0, maxRefined, "输入强化度(0-"..maxRefined..")"}})
		tinsert(tbOpt, {"(BT)瀚宇洪荒套..", AskClientForNumber, {"xqx_Equipment_getHanYuHongHuang", 0, maxRefined, "输入强化度(0-"..maxRefined..")"}})
	...
	tinsert(tbOpt, {"(BT)披风徽章相关 -->", function()
		...
		tinsert(tbOpt, {"获得风侠义令..", function()
			function tempfunc(nLevel)
				for i=1,15 do
					AddItem(0,155,100+i,1,1,-1,-1,-1,-1,-1,-1,-1,nLevel)
				end
			end
			local maxRefined = 15 + 229 * max(0,xqx.GetVipLevel()-1)    --用于计算不同权限允许最大精炼层数，GM权限=244，其他=15
			AskClientForNumber("tempfunc", 0, maxRefined, "输入强化度(0-"..maxRefined..")")
		...
		tinsert(tbOpt, {"获得花侠义令..", function()
			function tempfunc(nLevel)
				for i=1,15 do
					AddItem(0,155,200+i,1,1,-1,-1,-1,-1,-1,-1,-1,nLevel)
				end
			end
			local maxRefined = 15 + 229 * max(0,xqx.GetVipLevel()-1)    --用于计算不同权限允许最大精炼层数，GM权限=244，其他=15
			AskClientForNumber("tempfunc", 0, maxRefined, "输入强化度(0-"..maxRefined..")")
