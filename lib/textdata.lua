...
function TxtData:Init(szFile,szFileName)
	-- Check OS
	if strlower(strsub(getenv("os") or "",1,3)) == 'win' then
		self.DataFileName = "data\\"..szFile.."\\"..szFileName;
	else
		self.DataFileName = "data/"..szFile.."/"..szFileName;
	end
	self.tbTextData = {}
...
function TxtData:AddValue(nRow, Value, nMax)
	self:LoadData()
	-- Fill row
...
function TxtData:SetValue(nRow, Value)
	self:LoadData();
	nRow = nRow or 1;
	if getn(self.tbTextData) < nRow then
		for i=getn(self.tbTextData), nRow do
...
function TxtData:SplitString(szData)
...
function TxtData:LoadMultiColumn()
	local pf = openfile(self.DataFileName, "r");
	if pf ~= nil then
		local str = ""
		local nRow = 0
...
function TxtData:SaveMultiColumn()
	local pf = openfile(self.DataFileName, "w");
	for i=1,getn(self.tbTextData) do
		local szData = ""
...
-- Add new line
function TxtData:AddLine(szData)
