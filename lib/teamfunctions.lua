...
--检查队伍条件
--返回：1：符合
--    ：0, "MemberNum", szName：不符合
function gf_CheckTeamOptions(nTeamID, tOptions)
--	local tOptions = {
--		MemberNum,		--队伍成员	{min, max}
--		MemberRouteNum,	--队伍流派数量	{min, max}
--		MemberLevel,	--玩家等级	{min, max},
--		MemberHaveRoute,--成员是否已入流派 MemberHaveRoute = 1,
...
--关卡进入限制(记录次数的taskid， 次数上限)
function gf_CheckEnterLimit(nTaskID, nTimes)
	local tConditions = {
		["tTaskIdCond"]	= {"闯关已超过限制（每天" .. nTimes .. "次）", 1},
	};
