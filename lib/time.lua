...
function GetTimeHourStr(nTime)
	local nHour = floor(nTime / 60)
	local nMin	= mod(nTime, 60)
	local szTime = ""
	if nHour > 0 then
		szTime = format("%s%d小时", szTime, nHour)
	end
	if nMin > 0 or nHour <= 0 then
		szTime = format("%s%d分钟", szTime, nMin)
	end
	return szTime
end
...
--格式化时间类似于Lua自带的date函数
--nCurTime:时间点：没有输入表示返回当前日期（yyyymmdd）
function tf_IntDate(nCurTime)
	nCurTime = nCurTime or GetTime();
	local nYear,nMonth,nDay,nHour,nMin,nSec = GmTime(nCurTime+tf_GetTimeZoneInfo()*3600);
	local format_time = function(nData,nLen)
		nLen = nLen or 2;
		local szRet = nData;
		local nDataLen = strlen(nData);
