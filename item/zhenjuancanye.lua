...
function OnUse(nItemIdx)
	local nHave = GetItemCount(2,96,176)
	if nHave < 20 then
		Talk(1,"","您需要有<color=yellow>20<color>个[师门真卷残页]才可以合成1本[<color=yellow>师门真卷<color>]，再检查一下吧。")
		return
	end
	local selTab = {
				"同意/#use("..nItemIdx..")",
	...
	Say("您同意使用<color=yellow>20<color>个[师门真卷残页]来合成1本[<color=yellow>师门真卷<color>]吗？",getn(selTab),selTab);
end;
...
function use(nItemIdx)
	...
	if nRouteFlag ~= 1 then
		Talk(1,"","侠士还没加入任何流派，不能合成<color=yellow>师门真卷<color>。");
		return
	end;
	if DelItem(2,96,176, 20) == 1 then
		ahf_GetGaojiZhenjuan(1, 1, 1)
	end;	
end;
