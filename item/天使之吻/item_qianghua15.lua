npc_name = "<color=green>强化<color>: ";
THIS_FILE = "\\script\\item\\天使之吻\\item_qianghua15.lua";

function OnUse()
		--------------if GetCash() < 30000000 then
			--------------Talk(1,"","<color=green>强化道具<color>：您身上不足3000金。");
			--------------return
		--------------end
...
function force_item(tItem)
	...
	if d >= 113 and d <=117  then 
		Talk(1,"",npc_name.."纹饰无法强化请去天工巧手刘诗英")
		return 0;
	end		
	if nEnhance >= 15 then 
