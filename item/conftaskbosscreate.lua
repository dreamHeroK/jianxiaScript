...
function OnUse(id)
	...
	if chk_flag == 0 then
		Say("这里守卫严密, 步非烟的人无论如何都不敢来, 试试别的地方!",
			2,
			"我想查看哪些地图可使用生辰纲/Zgc_bfy_actmap_inq",
			"我知道了/Zgc_end_dialog"
		)		
	else														--创建BOSS	
...
--          if boss_step <= 100 then
				...
				SendTongMessage(GetName().."使用生辰纲召唤了步非烟!")
				Msg2SubWorld("帮会"..GetTongName()..",使用生辰纲召唤了步非烟!")
				AddLocalNews("帮会"..GetTongName()..",使用生辰纲召唤了步非烟!")
--				WriteLogEx(strLogCaption, "召唤", "1", "步非烟", "", GetTongName())
			elseif boss_step <= 50 then
				local boss_seq = random(5,6)
				npc_index = CreateNpc(Zgc_conf_task_boss[boss_seq],Zgc_conf_task_boss[boss_seq],mapID,X,Y,-1,1,0,0)
				SendTongMessage(GetName().."使用生辰纲召唤了:"..Zgc_conf_task_boss[boss_seq].."!")
