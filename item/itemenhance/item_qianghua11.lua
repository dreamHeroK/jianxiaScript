...
THIS_FILE = "\\script\\item\\itemenhance\\item_qianghua11.lua";

function OnUse()
		local MaxPutinCount = 1; --最多放一个
	PutinItemBox("装备强化11", MaxPutinCount, npc_name.."你确认你加入的物品了吗?", THIS_FILE, 1);
end;	
...
function force_item(tItem)
	...
	local nAdd = AddItem(g,d,p,1,4,nLv1,nMofa1,nLv2,nMofa2,nLv3,nMofa3,0,11,nLingqi)
	if nAdd == 1 then 
     DelItem(2,102,1111,1);
	else
