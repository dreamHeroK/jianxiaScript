...
THIS_FILE = "\\script\\item\\itemenhance\\item_qianghua14.lua";

function OnUse()
		if GetCash() < 20000000 then
			Talk(1,"","<color=green>强化道具<color>：您身上不足2000金。");
			return
		end

	local MaxPutinCount = 1; --最多放一个
	PutinItemBox("装备强化14", MaxPutinCount, npc_name.."你确认你加入的物品了吗?", THIS_FILE, 1);
end;	
...
function force_item(tItem)
	...
	local nAdd = AddItem(g,d,p,1,4,nLv1,nMofa1,nLv2,nMofa2,nLv3,nMofa3,0,14,nLingqi)
	if nAdd == 1 and  Pay(20000000) == 1then 
     DelItem(2,102,1114,1);
	else
