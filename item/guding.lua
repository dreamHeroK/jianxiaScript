...
g_szInfoHeader = "<color=green>神木王鼎<color>：";

function OnUse()
	local selTab = {
				"取出蛊/get_guinsect",
				"把包袱里的蛊全放入鼎中/put_guinsect",
				"观赏下蛊而已/nothing",
				}
	Say(g_szInfoHeader.."此鼎容纳了天下各种蛊虫，您想取出蛊还是放入蛊？",getn(selTab),selTab);
end;
...
function get_guinsect()
	...
	tinsert(selTab,"\n暂不取出/OnUse");
	Say(g_szInfoHeader.."您要取出哪系蛊？",getn(selTab),selTab);
end;
...
function list_page(nPageNum,nType)
	...
	tinsert(selTab,"\n暂不取出/get_guinsect");
	Say(g_szInfoHeader.."您取出哪种蛊？",getn(selTab),selTab);
end;
...
function input_guinsect_num(nType,nDetailType,nCurNum)
	...
	if nCurNum <= 0 then
		Talk(1,"get_guinsect",g_szInfoHeader.."你的蛊鼎中并没有<color=yellow>"..szGuInsectName.."<color>这种蛊。");
		return 0;
	end;
...
function get_guinsect_callback(nNum)
	...
	if nCurGuInsectNum < nNum then
		Talk(1,"get_guinsect",g_szInfoHeader.."你的蛊鼎中"..szGuInsectName.."的数量不足"..nNum.."只。");
		return 0;
	end;
	AddGuInsectIntoBottle(nGuInsectID,-nNum);
	AddItem(GU_INSECT_ID1,GU_INSECT_ID2,nGuInsectID,nNum);
	Msg2Player("你取出了"..nNum.."只"..szGuInsectName);
		...
		for j=1,getn(TB_GUINSECT_INFO[i]) do
			...
			if nGuInsectNum > 0 then
				if DelItem(GU_INSECT_ID1,GU_INSECT_ID2,nGuInsectID,nGuInsectNum) == 1 then
					...
					Msg2Player("你把"..nGuInsectNum.."个"..szGuInsectName.."放入了蛊鼎中");
				end;
			end;
		end;
	end;
	Say("您已经把包袱中的蛊全部放入了蛊鼎中！",0);
end;
