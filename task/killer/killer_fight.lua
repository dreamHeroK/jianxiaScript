...
function OnDeath(NpcIndex)
	...
	local nKillerParName = nKillerName.."杀手同伴";
	...
	if nState == 1 and nKillerIndex == NpcIndex then--判断杀死该杀手的玩家是否为领该任务的玩家	 
		...
		ModifyExp(nExp);
		Msg2Player("你获得了"..nExp.."点经验点");
		TaskTip("恭喜你完成杀手任务并获得奖励!");
		--移除时间触发器
		...
		if random(1,100) <= 30 then
			...
			if random(1,100) <= 100 then
				Talk(1,"","<color=green>"..nKillerName.."<color>：别以为杀掉我就算完了，我的同伴会为我报仇的！");
				...
				Talk(2,"","<color=green>"..nKillerName.."<color>：算你厉害！","在<color=yellow>"..nKillerName.."<color>身上搜取了一枚<color=yellow>杀手令牌<color>.");
				--掉落杀手令
				AddItem(2,1,195,1,1);
			end;
		else
			Talk(1,"","<color=green>"..nKillerName.."<color>：算你厉害！");
		end;
		...
		if random(1,100) <= 10 then
			...
			if nLSLevel ~= 0 then
				Msg2Player("恭喜你获得了1枚"..nLSLevel.."级灵石, 灵石已放入聚灵鼎中，请自行查看！");		
			end;
		end;
	end;
end;
