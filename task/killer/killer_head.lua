...
function CreateKiller()
	...
	--获取杀手姓名
	local nKillerName = "杀手"..getKillerName(nSurnameID,nFirstnameID);	
	--获取杀手模板ID
...
function GetTopKillerData()
	...
	if nTopKillerName ~= nil then
		nTopKillerName = "杀手头目"..nTopKillerName;
	else
		nTopKillerName = "杀手头目"
	end
	--获取玩家等级区域
...
function KillerAward()
	...
	local nExp = GetTask(TASK_PLAYER_LEVEL)*GetTask(TASK_PLAYER_LEVEL)*80;
	local nMoney = 2*10000
	local nReputation = 10
...
function TimeOut()
	...
	TaskTip("抱歉，你在指定时间内并没有到达指定地图，任务失败，通缉令消失!");
	TaskPunish();
...
function TaskPunish()
	...
	Msg2Player("你的杀手任务失败，扣除2点声望！");
	TaskTip("你的杀手任务失败，扣除2点声望");
end;
...
function TurnTime(nSec)
	...
	local str = "";
	str = " <color=yellow>"..nHour.."<color>时<color=yellow>"..nMinute.."<color>分<color=yellow>"..nSecond.."<color>秒 ";
	return str
end
