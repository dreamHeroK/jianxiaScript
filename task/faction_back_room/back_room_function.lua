...
function BR_NextStage()
	...
	if nCurStage == 1 or nCurStage == 5 or nCurStage == 9 then
		if BR_EnterNextStageCheck(nCurDiff,nCurStage,nPIdx) == 0 then
			Say2SomeOne(nPIdx,"您目前不能进行下一关，找接引弟子查看您的权限",0);
			Msg2MSAll(MISSION_ID,"您目前不能进行下一关，找接引弟子查看您的权限");
	...
	Msg2MSAll(MISSION_ID,""..TB_DIFF_NAME[nCurDiff].."难度第"..nCurStage.."关开始了");
...
function BR_EndStage()
	...
	if nEnterType == 1 then
		Msg2MSAll(MISSION_ID,"您已通过"..TB_DIFF_NAME[nCurDiff].."难度的第"..nCurStage.."关，读档进入，本难度无时间记录");
	else
		Msg2MSAll(MISSION_ID,"您已通过"..TB_DIFF_NAME[nCurDiff].."第"..nCurStage.."关，目前用时："..tf_GetTimeString(nUseTime)..".");
	end;
	if nCurStage == MAX_STAGE then	--如果是循环内最后一关
		...
		if nBeginTime ~= 0 then
			if nEnterType == 1 then
				Msg2MSAll(MISSION_ID,"恭喜您闯过"..TB_DIFF_NAME[nCurDiff].."难度，读档进入，本难度无时间记录");
				WriteLog(LOG_HEADER.."，难度："..nCurDiff..", 第"..nCurStage.."关。名称："..GetName(nPIdx)..", 等级："..GetLevel(nPIdx)..","..GetMissionV(MV_ROUTE).."，读档进入，本难度无时间记录");			
			else
				Msg2MSAll(MISSION_ID,"恭喜您闯过"..TB_DIFF_NAME[nCurDiff].."难度，所用时间："..tf_GetTimeString(nUseTime));
				if BR_UpDateTimeRecord(nCurDiff,nUseTime,nPIdx) == 1 then		--提交时间
					Msg2MSAll(MISSION_ID,"恭喜您打破自己"..TB_DIFF_NAME[nCurDiff].."难度的最快闯关记录");
				end;
				WriteLog(LOG_HEADER.."，难度： "..nCurDiff..", 第"..nCurStage.."关。名称："..GetName(nPIdx)..", 等级："..GetLevel(nPIdx)..","..GetMissionV(MV_ROUTE)..", 目前用时："..nUseTime);			
			end;
		end;
	end;
	...
	if nCurDiff == MAX_DIFFICULTY_LEVEL and nCurStage == MAX_STAGE then	--如果是最后循环最后关卡
		...
		Msg2MSAll(MISSION_ID,"恭喜您通过全部考验，不愧是"..szFactionName.."最出色的弟子，稍作等候，马上将您传送出去。")
		BR_FinalEndStage();
		return 0;
	end;
...
function BR_CreateDisciple()
	...
	local nNpcIdx = CreateNpc("王安石","接引弟子",nMapID,nMapX,nMapY);
...
function BR_InitStage(nDiff,nStage)
	...
	BR_StartTimeGuage("总时间",nTime,0,0);
	SetMissionV(MV_NSC_NEED_KILL_NUMBER,nNpcCount);
	Msg2MSAll(MISSION_ID,"本关时限："..tf_GetTimeString(nTime).."。过关所需击败守卫数量："..nNpcCount);
end;
--转换Mission当前状态
...
function BR_CreateAwardBox(nDiff,nStage,nPIdx)
	...
	local nNpcIdx = CreateNpc("箱子钱",""..TB_DIFF_NAME[nDiff].."难度第"..nStage.."关宝箱",nMapID,nMapX,nMapY);
...
function BR_RemoveNpcIndex(nNpcIdx)
	...
	gf_ShowDebugInfor("BR_RemoveNpcIndex 获取 nNpcIdx 不在记录量变量中 Mission");
	WriteLog(LOG_ERR_HEADER.."BR_RemoveNpcIndex 获取 nNpcIdx 不在记录量变量中 Mission");
	return 0;
end;
...
function BR_SaveRecord(nDiff,nStage,nPIdx)
	...
	if nDiff > MAX_DIFFICULTY_LEVEL then	--如果是最后循环最后关卡就不存档了
		...
		Msg2MSAll(MISSION_ID,"清档，本周第"..GetTask(TSK_WC_FAIL_TIMES).."次清档次数");
		return 0;
	end;
	local nRecord = nDiff*100+nStage;
	SetTask(TSK_WC_LAST_RECORD,nRecord);
	Msg2MSAll(MISSION_ID,"进度已保存："..TB_DIFF_NAME[nDiff].."难度第"..nStage.."关");
	PlayerIndex = nOldPIdx;
end;
--获得进度保存信息
...
function BR_CloseStage()
	...
	if UNCOMPLETED_VERSION ~= 1 then
		...
		if nRetCode ~= 0 then
			WriteLog(LOG_ERR_HEADER.."FreeDynamicMap 错误, nRetCode:"..nRetCode);
		end;
	end;
end;
...
function BR_ClearMapNpcSafe(nMapID)
	if BR_CheckMissionName() == 1 then
		...
		WriteLog(LOG_ERR_HEADER.."函数 BR_ClearMapNpcSafe 检查地图时发生错误");
		return 0;
	end;
end;
--开启时间进度条
...
function BR_GetBoxAward(nDiff,nStage)
	...
	for i=1,getn(tbDropInfo) do
		...
		if nCount > 0 then
			...
			Msg2Player("您获得了["..TB_TOTEM_INFO[nFaction][i][4].."]×"..nCount);
		end;
	end;
	local nEnterDiff = GetTask(TSK_ENTER_DIFF);	--从第几个难度进入
	if nDiff == 9 and nStage == 9 then
		if nEnterDiff == 1 then
--			local nID2, nID3, nID31, nID32 = 0, 0, 0, 0;
--			local nID2List = {100,101,103}
--			nID2 = nID2List[random(1,3)];
--			nID31 = random(22582,22593);
--			nID32 = random(22596,22623);
--			nID33 = random(22628,22629);
--			nID34 = random(22642,22645);
--			nID3 = random(nID31,nID32,nID33,nID34); 
--			local name = GetItemName(0,nID2,nID3)
--			AddItem(0,nID2,nID3,1,4,-1,-1,-1,-1,-1,-1,-1);
--			Msg2Player("您获得了["..name.."]×1");
--			Msg2Global("恭喜["..GetName().."]通过<师门密室>最后一关，获得["..name.."]×1");
		elseif nEnterDiff == 2 then
			BR_GetEnterDiffAward(1,4)
			BR_GetEnterDiffAward(1,8)
			BR_GetEnterDiffAward(1,9)
		elseif nEnterDiff == 3 then
			BR_GetEnterDiffAward(1,4)
			BR_GetEnterDiffAward(1,8)
			BR_GetEnterDiffAward(1,9)
			BR_GetEnterDiffAward(2,4)
			BR_GetEnterDiffAward(2,8)
			BR_GetEnterDiffAward(2,9)
		elseif nEnterDiff == 4 then
			BR_GetEnterDiffAward(1,4)
			BR_GetEnterDiffAward(1,8)
			BR_GetEnterDiffAward(1,9)
			BR_GetEnterDiffAward(2,4)
			BR_GetEnterDiffAward(2,8)
			BR_GetEnterDiffAward(2,9)
			BR_GetEnterDiffAward(3,4)
			BR_GetEnterDiffAward(3,8)
			BR_GetEnterDiffAward(3,9)
		elseif nEnterDiff == 5 then
			BR_GetEnterDiffAward(1,4)
			BR_GetEnterDiffAward(1,8)
			BR_GetEnterDiffAward(1,9)
			BR_GetEnterDiffAward(2,4)
			...
			BR_GetEnterDiffAward(3,8)
			BR_GetEnterDiffAward(3,9)
			BR_GetEnterDiffAward(4,4)
			BR_GetEnterDiffAward(4,8)
			BR_GetEnterDiffAward(4,9)
		elseif nEnterDiff == 6 then
			BR_GetEnterDiffAward(1,4)
			BR_GetEnterDiffAward(1,8)
			BR_GetEnterDiffAward(1,9)
			BR_GetEnterDiffAward(2,4)
			...
			BR_GetEnterDiffAward(4,8)
			BR_GetEnterDiffAward(4,9)
			BR_GetEnterDiffAward(5,4)
			BR_GetEnterDiffAward(5,8)
			BR_GetEnterDiffAward(5,9)
		end;
		SetTask(TSK_ENTER_DIFF,0);	--清除第一次难度进入
	end
...
--		gf_AddItemEx({2,111,170,nNum}, "千年藤枝")
	end;
--	gf_AddItemEx({2,111,170,10}, "千年藤枝")
	local nExp = 0;
	local nGongXian = 0;
	local nXiuWei = 0;
	...
	ModifyPopur(nXiuWei)
	Msg2Player("您获得"..nXiuWei.."点修为");
	local nCurGongXian = GetTask(336);
	if nCurGongXian > 12500 then
		ModifyReputation(nGongXian,0);
--		Msg2Player("您的师门贡献已超过12500分，不再获得师门贡献值");
	else
		SetTask(336,nCurGongXian+nGongXian);
		Msg2Player("您获得"..nGongXian.."点声望");
	end;
	Msg2Player("您拾取了"..TB_DIFF_NAME[nDiff].."难度第"..nStage.."关的宝箱");
	WriteLog(LOG_HEADER..GetName().." 通过关卡："..nDiff.."第"..nStage..".	");
end;

function BR_GetEnterDiffAward(nDiff,nStage)
	local tbDropInfo = TB_DROP_INFO[nDiff][nStage];
	local nFaction = GetPlayerFaction();
	for i=1,getn(tbDropInfo) do
		local nCount = tbDropInfo[i];
...
--	gf_AddItemEx({2,111,170,10}, "千年藤枝")
	...
	if nCurGongXian > 12500 then
		...
		SetTask(336,nCurGongXian+nGongXian);
		Msg2Player("您获得"..nGongXian.."点贡献值");
	end;
	Msg2Player("您获得了跳关前"..TB_DIFF_NAME[nDiff].."难度第"..nStage.."关的奖励");
	WriteLog(LOG_HEADER..GetName().." 跳关前关卡："..nDiff.."第"..nStage.."关奖励");
end;
...
function BR_CheckRouteFeature(nDiff,nStage)
	...
	{	--{{属性1,属性2},{45循环限制数值,67循环限制数值,89循环限制数值}}
		...
		[22] = {},			--昆仑
		[23] = {{{1,2},{180,225,370}}},--昆仑天师
		[24] = {},			--明教
		[25] = {{{1,2},{180,225,370}}},--明教圣战
		[26] = {{{3,4},{160,205,330}},{{4,5},{150,195,320}}},--明教阵兵
		[27] = {{{3,4},{160,205,330}},{{4,5},{150,195,320}}},--明教血人
		[28] = {},			--翠烟
		[29] = {{{1,2},{180,225,370}}},--翠烟舞仙
		[30] = {{{3,4},{160,205,330}},{{4,5},{150,195,320}}},--翠烟灵女
	}
	local nIndex = 0;
