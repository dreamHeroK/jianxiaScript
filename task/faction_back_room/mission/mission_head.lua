...
function MS_InitMission()
	...
	SetMissionS(MS_MISSION_STRING,"师门");
	WriteLog(LOG_HEADER.."Mission 开始, MSUID:"..nMSUID);
end;
--Mission开始
...
function MS_EndMission()
	...
	WriteLog(LOG_HEADER.."Mission 结束, MSUID:"..nMSUID);
	mf_ClearMissionData();
end;
--离开Mission
...
function MS_ProcessReadyTimer()
	...
	if nLoop == 0 then
		...
		Msg2MSAll(MISSION_ID,"准备时间结束，开始闯关……");
		BR_NextStage();
	else
		SetMissionV(MV_TIMER_LOOP,nLoop-1);
		Msg2MSAll(MISSION_ID,"准备阶段，时间还有"..tf_GetTimeString((nLoop*READY_TIMER_INTERVAL)*60));
	end;
end;
--处理Mission开始后的计时器函数
function MS_ProcessStartedTimer()
	...
	if nLoop == 0 then
		...
		BR_CreateDisciple();
		WriteLog(LOG_HEADER.."闯关失败："..nCurDiff.."难度第"..nCurStage.."关。姓名："..GetName(nPIdx)..", 等级："..GetLevel(nPIdx)..","..GetMissionV(MV_ROUTE)..", 原因：超时");
	else
		BR_ProcStageEven(nCurDiff,nCurStage);
		SetMissionV(MV_TIMER_LOOP,nLoop-1);
		if mod(nLoop,6) == 0 then
			Msg2MSAll(MISSION_ID,"闯关中，时间还有"..tf_GetTimeString(60*nLoop*STARTED_TIMER_INTERVAL));
		end;
	end;
end;
--处理过关短暂等待的计时器函数
function MS_ProcessShortPauseTimer()
	local nLoop = GetMissionV(MV_TIMER_LOOP);
	if nLoop == 0 then
		Msg2MSAll(MISSION_ID,"准备时间结束，进入下一关……");
		BR_NextStage();
	else
		SetMissionV(MV_TIMER_LOOP,nLoop-1);
		local nSec = 60*nLoop*SHORT_PAUSE_TIMER_INTERVAL;
		Msg2MSAll(MISSION_ID,"准备阶段，时间还有"..tf_GetTimeString(nSec));
	end;
end;
--处理等待结束的计时器函数
function MS_ProcessEndingTimer()
	local nLoop = GetMissionV(MV_TIMER_LOOP);
	if nLoop == 0 then
		Msg2MSAll(MISSION_ID,"时间到，现在请您出关……");
		BR_CloseStage();
	else
		SetMissionV(MV_TIMER_LOOP,nLoop-1);
		Msg2MSAll(MISSION_ID,"结束阶段，时间还有"..tf_GetTimeString(60*nLoop*ENDING_TIMER_INTERVAL));
	end;
end;
--登陆触发器函数
function OnLogin()
	MS_RestorePlayerState();
	WriteLog(LOG_HEADER.."玩家"..GetName().."离开Mission不正常，重新处理登录过程");
end;
--=====================================================================================
