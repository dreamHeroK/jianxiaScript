...
function VNG_Pet_Effect()
	local tNoiNgoai = {
			 [1] = {
						{"ÄÚ¹¥Ìá¸ß 6%", "state_m_attack_percent_add", 6},
						{"ÉúÃüÉÏÏÞÔö¼Ó +6000", "state_life_max_point_add", 8000},
						{"ÉúÃüÉÏÏÞÌá¸ß 6%", "state_life_max_percent_add", 6},
						{"ËùÓÐÊôÐÔÔö¼Ó +12", "state_add_allability", 12}, 
					},
			[2] = {
						{"Íâ¹¥Ìá¸ß 12%", "state_p_attack_percent_add", 12},
						{"ÉúÃüÉÏÏÞÔö¼Ó +6000", "state_life_max_point_add", 10000},
						{"ÉúÃüÉÏÏÞÌá¸ß 6%", "state_life_max_percent_add", 6},
						{"ÄÚÍâ¹¦·ÀÓùÔö¼Ó +60", "state_i_defence_point_add", 60},
						{"ÄÚÍâ¹¥·ÀÓùÔö¼Ó +60", "state_o_defence_point_add", 60},
					},
				}
	local nWay = mod(floor(GetTask(TSK_PET_PS_GIAIDOAN_A)/1000),10)
	local nAttrId = 2908130
	if mod(floor(GetTask(TSK_PET_PS_GIAIDOAN_B)/10),10) >= 1 then
...
function VNG_Del1Time()
	local tb_translife_cloth_PS_Xoa = {
		{"Çç¿Õ±ÌÏö", {0,108,1093,1,4,7,639,3,371,7,102}}, 
		{"Çç¿Õ±ÌÏö", {0,109,1093,1,4,5,342,7,102}}, 
		{"Çç¿Õ±ÌÏö", {0,110,1093,1,4,4,282,7,250,7,102}},
		{"Çç¿Õ±ÌÏö", {0,108,1093,1,4,7,27,7,102}}, 
		{"Çç¿Õ±ÌÏö", {0,109,1093,1,4,5,342,4,409,7,102}}, 
		{"Çç¿Õ±ÌÏö", {0,110,1093,1,4,5,292,5,293,7,102}},
		{"Çç¿Õ±ÌÏö", {0,108,1093,1,4,3,13075,7,102}}, 
		{"Çç¿Õ±ÌÏö", {0,109,1093,1,4,5,342,2,346,7,102}}, 
		{"Çç¿Õ±ÌÏö", {0,110,1093,1,4,4,282,7,250,7,102}},
		{"Çç¿Õ±ÌÏö", {0,108,1093,1,4,6,302,7,409,7,102}}, 
		{"Çç¿Õ±ÌÏö", {0,109,1093,1,4,5,13219,5,13220,7,102}}, 
		{"Çç¿Õ±ÌÏö", {0,110,1093,1,4,4,290,7,250,7,102}},
									}
	...
	for i=1, getn(tb_translife_cloth_PS_Xoa) do
			local G,D,P,nCount, _, lvatt1, att1, lvatt2, att2, lvatt3, att3  = 	gf_UnPack(tb_translife_cloth_PS_Xoa[i][2])
			for j=1, 4 do
	...
	gf_WriteLogEx("CHUYEN SINH 7", "Çç¿Õ±ÌÏö", 1, "Çç¿Õ±ÌÏö PS2")
end
...
function TaiXuatGiangHo_Init()
	...
	if nLeaveTime > 30 then
		...
		Talk(1,"","ÔÝÊ±Î´Öª01 <color=yellow>"..nLeaveTime.."Î´Öª02<color>Î´Öª03.")
	end
end
...
function VNG_ThangTienPhucSinh()
	...
	if nStatus == 2 then
		...
		gf_WriteLogEx("THANG TIEN PHUC SINH", "thang tien phuc sinh", 1, "thang tien phuc sinh")	
	end
end
