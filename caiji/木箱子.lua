...
nTotalItemCount = 7					-- 总共有多少采集物品

szItemGenre = 		{2,2,2,2,2,2,2,2,2}			-- 采集物品的大类
szItemDetail = 		{1,1,1,1,1,1,1,131,131}			-- 采集物品的副类
szItemParticular = 	{908,909,910,911,913,914,915,69,70}			-- 采集物品的详细类别
szItemGailv = 		{15,30,44,58,72,86,100}		-- 采集物品的概率（这里填的时候，后面一个要加上前面一个的概率）
szItemLowCount =	{20,20,20,20,20,20,20}		-- 采集物品最低数量
szItemHighCount =	{40,40,40,40,40,40,40}		-- 采集物品最高数量

nSkillType = 0						-- 采集类生活技能大类型
nSkillID = 7						-- 集灵技能ID
nSkillLevelLow = 20					-- 需要达到的采集技能最低等级
nSkillLevelHigh = 30				-- 需要达到的采集技能最高等级
...
function OnProgressCallback()
	...
	if (nChoice == 1) then
		Say("这个灵符很有效，你已经找到了"..nSelCount.."个武当符石！", 0)
	elseif (nChoice == 2) then
		Say("您的运气不错，包裹里有"..nSelCount.."个丐帮符石！", 0)
	elseif (nChoice == 3) then
		Say("您的运气不错，包裹里有"..nSelCount.."个少林符石！", 0)
	elseif (nChoice == 4) then
		Say("您的运气不错，包裹里有"..nSelCount.."个峨嵋符石！", 0)
	elseif (nChoice == 5) then
		Say("您的运气不错，包裹里有"..nSelCount.."个唐门符石！", 0)
	elseif (nChoice == 6) then
		Say("您的运气不错，包裹里有"..nSelCount.."个五毒符石！", 0)
	elseif (nChoice == 7) then
		Say("您的运气不错，包裹里有"..nSelCount.."个天波杨府符石！", 0)
	elseif (nChoice == 8) then
		Say("您的运气不错，包裹里有"..nSelCount.."个昆仑符石！", 0)
	elseif (nChoice == 9) then
		Say("您的运气不错，包裹里有"..nSelCount.."个翠烟符石！", 0)
	else
