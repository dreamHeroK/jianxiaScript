...
nTotalItemCount = 2					-- 总共有多少采集物品

szItemGenre = 		{2,2}			-- 采集物品的大类
szItemDetail = 		{10,2}			-- 采集物品的副类
szItemParticular = 	{7,23}			-- 采集物品的详细类别
szItemGailv = 		{85,100}		-- 采集物品的概率（这里填的时候，后面一个要加上前面一个的概率）
szItemLowCount =	{25,5}		-- 采集物品最低数量
szItemHighCount =	{40,8}		-- 采集物品最高数量

nSkillType = 0						-- 采集类生活技能大类型
nSkillID = 2						-- 制皮技能ID
nSkillLevelLow = 61					-- 需要达到的采集技能最低等级
nSkillLevelHigh = 70				-- 需要达到的采集技能最高等级

nDigToolCount = 3					-- 采集工具种类数量
nDigToolGenre = {0, 0, 0}				-- 采集工具大类
nDigToolDetail = {200, 200, 200}			-- 采集工具副类
nDigToolParticular = {41, 42, 43}			-- 采集工具详细类
nEndureCount = 20					-- 采集这个东西要消耗的耐久度
...
function OnProgressCallback()
	...
	-- 提示
	if (nChoice == 1) then
		Say("你已经采集了"..nSelCount.."个鸾鸟羽披！", 0)
	elseif (nChoice == 2) then
		Say("您的运气不错，包裹里有"..nSelCount.."个火浣锦！", 0)
	else
