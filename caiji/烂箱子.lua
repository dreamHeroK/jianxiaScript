...
nTotalItemCount = 6					-- 总共有多少采集物品

szItemGenre = 		{2,2,2,2,2,2,2}			-- 采集物品的大类
szItemDetail = 		{1,1,1,1,1,1,1}			-- 采集物品的副类
szItemParticular = 	{901,902,903,904,905,906,907}			-- 采集物品的详细类别
szItemGailv = 		{17,34,52,68,84,100}		-- 采集物品的概率（这里填的时候，后面一个要加上前面一个的概率）
szItemLowCount =	{20,20,20,20,20,20}		-- 采集物品最低数量
szItemHighCount =	{40,40,40,40,40,40}		-- 采集物品最高数量

nSkillType = 0						-- 采集类生活技能大类型
nSkillID = 7						-- 集灵技能ID
nSkillLevelLow = 1					-- 需要达到的采集技能最低等级
nSkillLevelHigh = 10				-- 需要达到的采集技能最高等级
...
function OnProgressCallback()
	...
	if (nChoice == 1) then
		Say("这个灵符很有效，你已经找到了"..nSelCount.."个汴京符石！", 0)
	elseif (nChoice == 2) then
		Say("您的运气不错，包裹里有"..nSelCount.."个成都符石！", 0)
	elseif (nChoice == 3) then
		Say("您的运气不错，包裹里有"..nSelCount.."个泉州符石！", 0)
	elseif (nChoice == 4) then
		Say("您的运气不错，包裹里有"..nSelCount.."个襄阳符石！", 0)
	elseif (nChoice == 5) then
		Say("您的运气不错，包裹里有"..nSelCount.."个凤翔符石！", 0)
	elseif (nChoice == 6) then
		Say("您的运气不错，包裹里有"..nSelCount.."个大理符石！", 0)
	elseif (nChoice == 7) then
		Say("您的运气不错，包裹里有"..nSelCount.."个扬州符石！", 0)
	else
