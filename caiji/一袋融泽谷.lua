...
szItemGenre = 		{1,1}			-- 采集物品的大类
szItemDetail = 		{3,3}			-- 采集物品的副类
szItemParticular = 	{12,22}			-- 采集物品的详细类别
szItemGailv = 		{50,100}		-- 采集物品的概率（这里填的时候，后面一个要加上前面一个的概率）
szItemLowCount =	{20,20}		-- 采集物品最低数量
szItemHighCount =	{30,30}		-- 采集物品最高数量

nSkillType = 0						-- 采集类生活技能大类型
nSkillID = 3						-- 收耕技能ID
nSkillLevelLow = 31					-- 需要达到的采集技能最低等级
nSkillLevelHigh = 40				-- 需要达到的采集技能最高等级

nDigToolCount = 3					-- 采集工具种类数量
nDigToolGenre = {0, 0, 0}				-- 采集工具大类
nDigToolDetail = {200, 200, 200}			-- 采集工具副类
nDigToolParticular = {41, 42, 43}			-- 采集工具详细类
nEndureCount = 15					-- 采集这个东西要消耗的耐久度
...
function OnProgressCallback()
	...
	-- 提示
	if (nChoice == 1) then
		Say("你已经采集了"..nSelCount.."袋融泽谷！", 0)
	elseif (nChoice == 2) then
		Say("您的运气不错，包裹里有 "..nSelCount.."胡椒！", 0)
	else
