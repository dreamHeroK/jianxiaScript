...
--	local selTab = {};
--	local szTitle = ""
--	local szTitle = format("%s世人皆浊，何不淈其泥而扬其波？可笑可笑。侠士当前气力值为：%d/%d。", strTitle, 600, 600)

--	if (GetLifeSkillLevel(nGen, nSkillId) > 0) then
--		tinsert(selTab,"进阶大师级钓鱼技能/update_max_skill_level");
--		tinsert(selTab,"我要了解采集的相关信息/Info");
--		tinsert(selTab,"送我去钓鱼地图/GotoWorld_Confirm");
--		tinsert(selTab,"遗忘钓鱼技能/forget_life_skill");
--	else
--		tinsert(selTab,"学习钓鱼技能/luaLearnLifeSkill");
--		tinsert(selTab,"我要了解采集的相关信息/Info");
--	end
...
function luaLearnLifeSkill()
	...
	Say(strTitle.."<color=yellow>钓鱼<color>将会成为你新的一个生活技能，你确定学习吗？",
    	2,
    	"是的，我要学/LearnYes",
    	"让我再考虑考虑/LearnNo")
end;
...
function LearnYes()
	if (GetLifeSkillLevel(nGen, nSkillId) > 0) then
		...
		if (LearnLifeSkill(nGen, nSkillId, 1, 79, 0)) then
			Talk(1,"", strTitle.."已经学会了[钓鱼]技能。你可以雇佣采集工人，开始采集之旅。！")
        		Msg2Player("你学会了生活技能：钓鱼")
		end
	end
end
...
function update_max_skill_level()
	...
	if GetLifeSkillMaxLevel(nGen, nSkillId) > 79 then
		Talk(1,"", strTitle.."您已进阶大师级钓鱼技能水平，不用再找我了！")
		return
	end

	local selTab = {};
	local szTitle = format("%s进阶大师级钓鱼技能要求钓鱼技能达到79级并消耗%d金，你确定要进阶吗？", strTitle, 1000)
...
function update_max_skill_level_ensure()
	if GetLifeSkillLevel(nGen, nSkillId) < 79 then
		Talk(1,"",format("%s目前钓鱼技能小于%d级，先去历练一番，再来领会。！", strTitle, 79))
		return 0;
	end
	...
	Msg2Player("恭喜进阶大师级钓鱼技能水平达到了99级");
...
function GotoWorld_Confirm1()
	Say(strTitle.."您是否要去<color=yellow>空桑山<color>钓鱼？那地方很多<color=yellow>鲈鱼<color>, <color=yellow>鲤鱼<color>, <color=yellow>桂鱼<color>和<color=yellow>白珍珠<color>，记得带上<color=yellow>神农丹<color>！",
		2,
		"我要去/#GotoWorld(712,1472,2918)",
		"稍后再说/exit_dialog")
end;
function GotoWorld_Confirm2()
	Say(strTitle.."您是否要去<color=yellow>昆吾山<color>钓鱼？那地方很多<color=yellow>鲶鱼<color>, <color=yellow>黑鱼<color>, <color=yellow>鲫鱼<color>和<color=yellow>黑珍珠<color>，记得带上<color=yellow>神农丹<color>！",
		2,
		"我要去/#GotoWorld(717,1736,2906)",
		"稍后再说/exit_dialog")
end;
function GotoWorld_Confirm3()
	Say(strTitle.."您是否要去<color=yellow>紫桑山<color>钓鱼？那地方很多<color=yellow>黄鱼<color>, <color=yellow>蟹<color>, <color=yellow>虾<color>和<color=yellow>河豚<color> ，记得带上<color=yellow>神农丹<color>！",
		2,
